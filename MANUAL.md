...
space, not all commands are listed on every screen where they're
available. For example, the search functions, which are available
everywhere, are summarized only in the packet list and address book. The
idea, which is not applied everywhere, is that the summary need appear
only on the first screen where the commands are available. When in doubt,
...
It is best to leave the "From" field as it is. For a BBS packet, the name
comes from the packet itself. The BBS built that packet for your account,
so the name already matches your name on the board. If you type another
name, the board often ignores it and uses your account instead. Synchronet
does this. For SOUP, the name comes from the "UserName" setting in
the .ncmailrc instead. If a message needs to carry another name, write
that name in the text of the message.
...
below the current one. You can use this to adjust the starting point for
the next search.
...
when updating the .red flags, so it can be slow. Also, the supplied
...
(simple) external script. NoCarrierMail only checks for the gzip
